...
import { readMediaText } from './webviewLoader';
...
export function transportPickerScript(): string {
    // Single source of truth: media/queueEditor/transportPicker.js. The queue
    // editor loads it directly as a <script src>; any other consumer that
    // inlines the picker wiring into its own script block gets the same text
    // here (Phase B.13 webview restructuring).
    return readMediaText('queueEditor', 'transportPicker.js');
}
