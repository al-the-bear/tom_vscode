 ...
 *
 * Telegram settings are stored **per quest and per host** at
 * `_ai/quests/{questId}/telegram.{hostname}.{questId}.yaml` — not in the shared
 ...
 * workspace/quest drive its own bot. The `_ai` clone is itself shared/symlinked
 * across the fleet, so the hostname segment keeps each machine's bot config
 * separate (each host runs its own bot token / chat).
 *
...
/**
 * Path to the per-quest, per-host Telegram settings file:
 * `_ai/quests/{questId}/telegram.{hostname}.{questId}.yaml`. Returns
...
export function getQuestTelegramConfigPath(): string | undefined {
    ...
    return WsPaths.ai('quests', questId, `telegram.${WsPaths.hostSlug()}.${questId}.yaml`);
}
...
function readQuestTelegramRaw(): any | undefined {
    ...
    try {
        return parseYaml(fs.readFileSync(questPath, 'utf-8'));
    } catch (err: any) {
...
export function writeQuestTelegramRaw(raw: any): boolean {
    ...
    try {
        ...
        fs.writeFileSync(questPath, stringifyYaml(out), 'utf-8');
        return true;
    } catch (err: any) {
 ...
 *
 * Resolution order:
 *   1. Per-quest, per-host file `_ai/quests/{questId}/telegram.{hostname}.{questId}.yaml` (authoritative when present).
...
export function loadTelegramConfig(): TelegramConfig | undefined {
    ...
    const fileName = questPath ? path.basename(questPath) : 'telegram.<host>.<quest>.yaml';
