...
function renderEntry(item, idx) {
  ...
  if (isEditable) {
    ...
    '<div class="repeat-affix-row">' +
      '<label style="font-size:0.8em;opacity:0.9;">Repeat Prefix (tokens: repeatNumber, repeatIndex, repeatCount)</label>' +
      '<textarea onchange="updateItemRepeat(\\'' + safeId + '\\', { repeatPrefix: this.value })">' + escapeHtml(repeatPrefix) + '</textarea>' +
      '<label style="font-size:0.8em;opacity:0.9;">Repeat Suffix (tokens: repeatNumber, repeatIndex, repeatCount)</label>' +
