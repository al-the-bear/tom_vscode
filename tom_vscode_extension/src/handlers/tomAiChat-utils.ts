...
export function buildSystemPrompt(): string {
    ...
    const basePrompt = [
        ...
        '- When you need a decision, clarification, missing information, or approval, call the `tomAi_askUser` tool — this is the ONLY way to ask the user and actually receive an answer.',
        '- Do NOT just write the question in your text reply: that reply is not surfaced to the user as a prompt, so nothing will answer it and the run will stall.',
        '- Syntax: `tomAi_askUser` with `questions` (an array of 1–15 strings) and an optional `title`, e.g. `{ "questions": ["Which target?", "OK to overwrite?"], "title": "Plan" }`. It blocks until the user answers (question panel or Telegram) or times out.',
        '',
        'Todo list (optional - use tomAi_manageTodo for complex multi-step tasks):',
        '- For simple tasks: Skip todos and just do the work.',
        '- For complex tasks with 3+ steps: Consider using tomAi_manageTodo to track progress.',
