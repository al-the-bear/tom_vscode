...
**Logs** is a read-only viewer over the active quest's own files (`questLogs-handler.ts` + `media/questLogs/`), with thirteen sub-tabs: MD Trail, Trail, Prompts, Answers, Progress, Overview, Notes, Refresh, DocUpdate, Deferred, Questions, Decisions, Current Prompt. Three of them are **journals** the assistant appends to as work happens: `deferred.<quest>.md` (steps put off), `questions.<quest>.md` (every `askUser` / `askUserPicker` question, every intercepted Agent SDK `AskUserQuestion`, and the answer given), `decisions.<quest>.md` (the `decisions[]` block of each todo archived as `completed`, with its id and archive time). The tab catalogue is the pure table `QUEST_LOG_TABS` in `utils/questLogFiles.ts`; each entry declares its `view` (`rendered` markdown vs. highlighted source), its `newestAt` end, and its `area` via `questLogLocation` (`quest` for the quest folder, `trail` for the gitignored Anthropic trail bucket). A file larger than 256 KB is read as a window from that end — the **tail** for the appended-to files (both `live-trail.md` tabs and the three journals) and the **head** for the quest documents that are prepended to or rewritten wholesale — and the view opens scrolled to it, staying pinned there while the reader has not scrolled away. Adding a sub-tab means adding one row to `QUEST_LOG_TABS` and one `case` to `questLogLocation`; the tab strip, the client-side id list and the read/anchor behaviour all derive from the table.

**Current Prompt** is the one sub-tab backed by several files, so it declares `variants` and the toolbar shows a dropdown while it is active. The six options are `{chat, queue} × {literal, user, system}` — two prompts can be in flight at once, and the three parts are what the user typed, the user message actually sent, and the system prompt. `AnthropicHandler.sendMessage()` calls `writeCurrentPrompt()` (`services/current-prompt-dump.ts`) next to the `writeRawPrompt` trail write, which is above every transport dispatch and the only point where all three texts are in scope; `literal` is `options.userText` rather than the keyword-stripped `effectiveUserText`, since the gap between the two is what the tab exists to show. All three files are rewritten on every send — an empty system prompt is written as an empty file rather than skipped, so nothing survives from the previous prompt. They live under `_ai/trail/anthropic/<quest>/` because the quest folder is tracked in the fleet-shared `_ai` repo and a per-send rewrite there would conflict on every prompt. Because a tab is no longer one file, the handler's change-detection cache is keyed by `questLogViewKey(tab, variant)` and every reply echoes the variant so a reply in flight when the dropdown changed is discarded.
