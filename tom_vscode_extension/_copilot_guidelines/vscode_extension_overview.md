...
**Logs** is a read-only viewer over the active quest's own files (`questLogs-handler.ts` + `media/questLogs/`), with ten sub-tabs: MD Trail, Trail, Prompts, Answers, Progress, Overview, Notes, Refresh, DocUpdate, Deferred. The tab catalogue is the pure table `QUEST_LOG_TABS` in `utils/questLogFiles.ts`; each entry declares its `view` (`rendered` markdown vs. highlighted source) and its `newestAt` end. A file larger than 256 KB is read as a window from that end — the **tail** for the appended-to files (both `live-trail.md` tabs and `completion_steps.<quest>.md`) and the **head** for the quest documents that are prepended to or rewritten wholesale — and the view opens scrolled to it, staying pinned there while the reader has not scrolled away. Adding a sub-tab means adding one row to `QUEST_LOG_TABS` and one `case` to `questLogFileName`; the tab strip, the client-side id list and the read/anchor behaviour all derive from the table.
