...
- **"Pause after this" holds the queue at item granularity, not stage granularity.** `item.pauseAfter` never shortens an item: the gate is consulted only where an item reaches `'sent'`, so all pre-prompts, every main-prompt repetition and all follow-ups run first. `decidePauseAfterCompletion` (`src/utils/queueStep3Utils.ts`) owns the decision and is called through the manager's `_holdQueueForPauseAfter`.
  - **`holdQueue` is independent of `autoSendEnabled`.** Three of the six completion sites (`sendItem`, `_resumePausedSendingItem`, `resendLastPrompt` — all Anthropic-transport tails) call `sendNext()` without consulting the auto-send flag, so flipping the flag off would not have stopped them. The gate returns the hold separately and each site skips its own advance.
  - **All six completion sites are covered**: answer-wait timer, answer-file watcher, resume-from-pause, manual **Continue** (`advanceSendingItemWithoutAnswer`), `sendItem`, `resendLastPrompt`. An item must not slip past the pause because of *how* it finished.
