...
# for this host only" — built straight into the extension's local bin/, with no
# reliance on $TOM_BINARY_PATH or any tool on PATH. Everything else (Node check,
# npm install, TypeScript compile, Claude Agent SDK host binary, vsce package,
# install) is shared with install_extension.sh.
